...
java %OPTIONS% -cp %TOOL_JAR% cloneworks.NiCadConvertor cloneworks-bigclonebench-blind.files cloneworks-bigclonebench-blind.clones %BIGCLONEBENCH_DIR%
java %OPTIONS% -cp %TOOL_JAR% drdup.Sourcer %SRC_DIR% cloneworks-bigclonebench-blind.xml

java %OPTIONS% -jar %CHECKER_JAR% checker.properties

echo "All clones"
java %OPTIONS% -cp %TOOL_JAR% drdup.Counter cloneworks-bigclonebench-blind.xml
echo "Type 1"
